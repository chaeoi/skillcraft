...
面向 Codex 的个人技能库（Skills Repository）。收录日常开发、技术汇报与深度工程方案撰写中高频使用的专业能力与工作流，所有技能均采用直观、通用的单单词（如 `ppt`、`code`、`word`）进行命名与分发。
...
└── skills/                    # 核心技能目录（单单词规范）
    ...
    │   └── references/
    │       └── guide.md       # 详细工程准则与工作流
    └── word/                  # 深度技术方案与工程规范撰写技能
        ├── SKILL.md           # 核心闭环准则、去AI感与排版规范
        ├── README.md          # 技能详细说明
        ├── agents/
        │   └── openai.yaml    # Codex 界面元数据与调用策略
        └── references/
            ├── guide.md       # 深度撰写指南（六要素闭环与三层边界）
...
| `word` | `skills/word` | 严谨工程方案与深度技术规范撰写，注重真实代码对齐、六要素步骤闭环与原生 OMML 公式排版 | `$word` |
...
面向严肃工程技术规范、系统架构方案与深度技术报告撰写。

- **核心特色**：严格六要素逻辑闭环（输入 $\to$ 处理 $\to$ 公式 $\to$ 判定 $\to$ 输出 $\to$ 异常分支）、公式参数**在正下方即刻逐一解释**、区分已实现与待确认边界、真实可计算数值实施例，以及基于 OMML 原生矢量公式彻底杜绝方框乱码。

```text
使用 $word 按照严谨闭环规范，将当前核心算法整理为一份高严谨度、高可读性的工程技术方案。
```
